...
uint16_t CRC::crc16_ccitt_update(uint16_t crc, uint8_t data) {
    uint8_t index = ((crc >> 8) ^ data) & 0xFF;
    return ((crc << 8) ^ CRC16_CCITT_TABLE[index]) & 0xFFFF;
}
...
uint32_t CRC::crc32_update(uint32_t crc, uint8_t data) {
